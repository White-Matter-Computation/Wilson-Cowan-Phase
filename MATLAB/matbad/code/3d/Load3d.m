...
legend_added = false(3,1);   % avoid duplicate legend entries
...
for k = 1:n_tau0
    ...
    for s = 0:2
        ...
        sc = scatter3(omegas_k(idx), tau1s_k(idx), tau0_k(idx), ...
            sizes(s+1), ...
            'Marker',          markers{s+1}, ...
        ...
        if legend_added(s+1)
            sc.HandleVisibility = 'off';   % only first occurrence in legend
        else
...
xlabel('\Omega'); ylabel('\tau_1'); zlabel('\tau_0');
title('Stability in (\Omega, \tau_1, \tau_0) space');
