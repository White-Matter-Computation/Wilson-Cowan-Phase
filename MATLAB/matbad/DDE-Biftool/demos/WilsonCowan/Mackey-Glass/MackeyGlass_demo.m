...
parnames={'w_ee','w_ei','w_ie','w_ii','p_e','p_i','tau_e','tau_i','tau_0','tau_1'};
...
bounds={'max_bound',[],'max_step',[0,0.3]};
...
N = 11;
w_k = @(k,N) if k==0, 0; else exp(-min(abs(k), N - abs(k)) / 2);¨
...
for j in 1:N
            utij = h(p, t - tau_k(i - j, n_neurons, tau_1))[2j-1]
            coupling += w_k(i - j, n_neurons) * utij
        end


du[1] = (-x(1,1) + f(par(ind.p_e) + par(ind.w_ee) * xtau(1,1) - par(ind.w_ei) * xtau(2,1))) / par(ind.tau_e)
du[2] = (-x(2,1) + f(par(ind.p_i) + par(ind.w_ie) * xtau(1,1) - par(ind.w_ii) * xtau(2,1))) / par(ind.tau_i)

rhs=@(x,xtau,par)   [];
fvec=set_funcs(...
    'sys_rhs',@(xx,p)rhs(xx(1,1,:),xx(1,2,:),p(ind.beta),p(ind.n)),...
    'sys_tau',@()ind.tau,'x_vectorized',false);
fsingle=set_funcs(...
    'sys_rhs',@(xx,p)rhs(xx(1,1,:),xx(1,2,:),p(ind.beta),p(ind.n)),...
    'sys_tau',@()ind.tau);
